...
using src.Applications.Services.Impls;
...
public static class DependencyExtension
{
    ...
    private static void SettingInfrastructures(IServiceCollection services)
    {
        ...
        // ドメインモデル:従業員ステータスとエンティティの相互変換インターフェイスの実装
        services.AddScoped<EmpStatusEntityAdapter>();
        // ドメインオブジェクト:部署のCRUD操作インターフェイス実装
        services.AddScoped<IDepartmentRepository, DepartmentRepository>();
        // ドメインオブジェクト:従業員のCRUD操作インターフェイスの実装
        services.AddScoped<IEmployeeRepository, EmployeeRepository>();
        // ドメインオブジェクト:従業員ステータスのCRUD操作インターフェイス実装
        services.AddScoped<IEmpStatusRepository, EmpStatusRepository>();
    ...
    private static void SettingApplications(IServiceCollection services)
    {
        ...
        ///従業員一覧サービスインタフェースの実装
        services.AddScoped<IEmployeeShowListService, EmployeeShowListService>();
        ///部署一覧サービスインタフェースの実装
        services.AddScoped<IDepartmentShowListService, DepartmentShowListService>();
    ...
    private static void SettingPresentations(IServiceCollection services)
    {
        ...
        // 従業員一覧ViewModelをドメインオブジェクト:部署に変換するアダプターインターフェイスの実装
        services.AddScoped<EmployeeShowListViewModelAdapter>();
        // TempDataへのEmployeeShowListViewの保存・復元するためのクラス
        // コンストラクタを利用して明示的にDIコンテナにインスタンスを登録する
        services.AddScoped(
            provider =>
            new TempDataStore<EmployeeShowListViewModel>("EmployeeShowListViewModel")
        );

        // 部署一覧ViewModelをドメインオブジェクト:部署に変換するアダプターインターフェイスの実装
        services.AddScoped<DepartmentShowListViewModelAdapter>();
        // TempDataへのDepartmentShowListViewの保存・復元するためのクラス
        // コンストラクタを利用して明示的にDIコンテナにインスタンスを登録する
        services.AddScoped(
            provider =>
            new TempDataStore<DepartmentShowListViewModel>("DepartmentShowListViewModel")
        );
    }
}